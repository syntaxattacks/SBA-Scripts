...
$NewUser = Read-Host 'Userid of the user you are copying to'
...
$NewUserName = Get-ADUser $NewUser -Properties DisplayName | Select Displayname |
  Format-Table -HideTableHeaders | Out-String
$NewUserName2 = Get-ADUser $NewUser -Properties DisplayName | Select Displayname |
...
$Groups = Get-ADPrincipalGroupMembership $OldUser |
  ...
  Where-Object {$_.managedby -ne "" -and $_.managedby -ne "Benson Richeme"} | 
...
$ManagerName = get-aduser -filter "displayname -like '$($Group.name)'" -properties GivenName | 
  Select GivenName | Format-Table -HideTableHeaders | Out-String
...
$Outlook = New-Object -ComObject Outlook.Application
  ...
  $Mail.To = $ManagerMail
  $Mail.Subject = "Security Group Access Request - $newuser"
  $Mail.HTMLBody ="$Greeting $ManagerName<br> <br>
  Please approve/deny <b>$NewUserName</b> for access to the following group(s):<br><br>
  $Groupnames<br><br>
  Their title is <b>$NewUserTitle</b> and they work in <b>$NewUserDepartment</b> for <b>$NewUserManager
  </b><br>"
  $Mail.Display()
  }
